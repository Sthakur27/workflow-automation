...
import { createMockQueryResult, createMockUuid, createEmptyMockQueryResult, createMockIntegrationResult } from '../utils/testUtils';
...
const mockExecuteIntegration = executeIntegration as jest.MockedFunction<typeof executeIntegration>;
...
describe('Run Service', () => {
  ...
  describe('triggerWorkflow', () => {
    it('should trigger a workflow and create a run', async () => {
      ...
      // Mock database query for workflow steps
      mockQuery.mockResolvedValueOnce(createMockQueryResult(mockWorkflow.steps));
      
      // Mock database query for inserting the run
    ...
    it('should return null if no workflow is found for the trigger', async () => {
      ...
      // Call the service
      const result = await runService.triggerWorkflow('webhook', 'non-existent');
      ...
      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining('No workflows found for trigger type webhook and value non-existent')
      );
    });
    ...
    it('should handle database errors', async () => {
      ...
      await expect(runService.triggerWorkflow('webhook', 'test-webhook')).rejects.toThrow();
  ...
  describe('getWorkflowRuns', () => {
    it('should return all runs for a workflow', async () => {
      ...
      // Verify database was queried
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining('SELECT * FROM workflow_runs WHERE workflow_id = $1'),
        ['workflow-id']
      );
  ...
  describe('retryWorkflowRun', () => {
    it('should retry a failed workflow run', async () => {
      ...
      // Mock database query for getting the step runs
      mockQuery.mockResolvedValueOnce(createMockQueryResult([{
        id: 'step-run-id',
        workflow_run_id: 'original-run-id',
        workflow_step_id: 'step-id',
      ...
      expect(mockWorkflowService.getWorkflow).toHaveBeenCalledWith('workflow-id');
    ...
    it('should return null if workflow is not found', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        expect.stringContaining('Workflow not found for run')
      );
    });
  ...
  describe('executeStepSequence', () => {
    ...
    it('should execute steps in sequence and handle input mappings', async () => {
      ...
      mockExecuteIntegration.mockResolvedValueOnce(createMockIntegrationResult(true, { items: [{ id: 123 }] }));
      mockExecuteIntegration.mockResolvedValueOnce(createMockIntegrationResult(true));
